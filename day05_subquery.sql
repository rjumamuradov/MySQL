...
-- select sehir, isim,id from calisanlar where
...
select marka_id,marka_isim, calisan_sayisi from markalar
...
select marka_isim,calisan_sayisi, (select sum(maas) from calisanlar
 ...
 select marka_isim,calisan_sayisi, (select AvG(maas) from calisanlar
 where marka_isim=isyeri) ortlama
 from markalar;
